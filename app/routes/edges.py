...
def get_today_edges(db: Session = Depends(get_db)):
    today = datetime.now(ET).date()
    # Get all games for today to ensure data is returned for each
    ...
    results = []
    for game in games:
        edge = latest_by_game.get(game.game_id)
        ...
        results.append({
            ...
            "ev_away": float(edge.ev_away) if edge and edge.ev_away is not None else None,
            "ev_home": float(edge.ev_home) if edge and edge.ev_home is not None else None,
            "ev_over": float(edge.ev_over) if edge and edge.ev_over is not None else None,
            "ev_under": float(edge.ev_under) if edge and edge.ev_under is not None else None,
            "confidence": edge.confidence_tier if edge else None,
            "movement_direction": edge.movement_direction if edge else None,
            "model_away_win_pct": float(edge.model_away_win_pct) if edge and edge.model_away_win_pct is not None else None,
            "model_home_win_pct": float(edge.model_home_win_pct) if edge and edge.model_home_win_pct is not None else None,
            "model_total": float(edge.model_total) if edge and edge.model_total is not None else None,
            "book_total": float(edge.book_total) if edge and edge.book_total is not None else None,
            "calculated_at": edge.calculated_at.isoformat() if edge and edge.calculated_at else None,
            # Advanced Features
            "kbb_adv": float(features.get("kbb_adv", 0.0)),
            "pythagorean_win_pct_adv": float(features.get("pythagorean_win_pct_adv", 0.0)),
            "park_factor_adv": float(features.get("park_factor_adv", 0.0)),
        })
    return results
